...
// TO DO: Replace with "Log" when ready
//------------------------------------------------------------------------
class FontLog
{
public:
    ...
    void LogWarning(const char* pfmt, ...);
    void LogInfo(const char* pfmt, ...);

private:
    void logMessageVarg(MessageType messageType, const char* pfmt, va_list argList);

    static const char* MessageHeader[EndOdMessageTypes];
};


//------------------------------------------------------------------------
class GlyphRaster : public RefCountBaseNTS<GlyphRaster, Stat_Default_Mem> // TO DO: StatID
{
public:
    GlyphRaster() : Width(0), Height(0), OriginX(0), OriginY(0) {}
...
//------------------------------------------------------------------------
class Font : public RefCountBase<Font, Stat_Default_Mem> // TO DO: StatID
{
    friend class FontCacheHandleManager;
...
public:
    ...
    inline UInt16 GetLowerCaseTop(FontLog* log)
    { 
        if (LowerCaseTop == 0)
            calcLowerUpperTop(log);
        return (LowerCaseTop <= 0) ? 0 : LowerCaseTop; 
    }
    inline UInt16 GetUpperCaseTop(FontLog* log) 
    { 
        if (UpperCaseTop == 0)
            calcLowerUpperTop(log);
...
protected:
    ...
    void   calcLowerUpperTop(FontLog* log);
...
//------------------------------------------------------------------------
class FontProvider : public RefCountBase<FontProvider, Stat_Default_Mem>
{
public:

...
{
    for(UPInt i = 0, n = Elements.GetSize(); i < n;)
    {
    ...
    case Cmd_InsertPlainText:
        {
            ...
            res = pDocument->InsertString(cmd.pStr, cmd.PosAt, len, StyledText::NLP_CompressCRLF);
            break;
        }
    ...
    case Cmd_ReplaceTextByChar:
        {
            ...
            if (len > 0)
            {
                res = pDocument->InsertString(&cmd.CharCode, stPos, 1, StyledText::NLP_CompressCRLF);
    ...
    case Cmd_ReplaceTextByPlainText:
        {
            ...
            res = pDocument->InsertString(cmd.pStr, stPos, len, StyledText::NLP_CompressCRLF);
    ...
    if (rect != ViewRect)
    {
        ...
        if (ut == UseExternally)
        {
            ...
            if (GetTextAutoSize() != TAS_None ||
                ...
                // check height...
                (oldH != newH && ((GetVAlignment() != VAlign_None && GetVAlignment() != VAlign_Top) || 
...
bool ParagraphFormatter::HandleCustomWordWrap()
{
    ...
    if (HyphenationRequested)
    {
        ...
        //grec.SetAdvance((int)(LineCursor.pLastFont->pFont->GetAdvance(dashGlyphIndex) * scale));
        ...
        LineCursor.LastGlyphWidth   = (int)(LineCursor.pLastFont->pFont->GetGlyphWidth(dashGlyphIndex) * Scale);
...
void DocView::Format()
{
    ...
    if (!valignApplied && valign != VAlign_None && !IsAutoSizeY())
    {
        ...
        {
            mLineBuffer.SetVisibleRect(maxTextRect);
        }
    }
...
//@REN
/*
void    DocView::PreDisplay(DisplayContext &context,
...
void    DocView::Display(DisplayContext &context,
                                const Matrix& mat, const Cxform& cx,
                                bool nextFrame)
{
    if (IsReformatReq())
    ...
    if (pHighlight)
    {
        ...
        mLineBuffer.Display(context, mat, cx, nextFrame, param, &pHighlight->HighlightManager);
    }
    else
    {
        mLineBuffer.Display(context, mat, cx, nextFrame, param);
    }
}

void    DocView::PostDisplay(DisplayContext &context,
                                    const Matrix& mat, const Cxform& cx,
                                    bool focused)
{
    SF_UNUSED4(context, mat, cx, focused);
    if (pHighlight)
    {
...
void DocView::SetSelection(UPInt startPos, UPInt endPos)
{

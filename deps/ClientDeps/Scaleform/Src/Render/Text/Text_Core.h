...
struct ImageDesc : public RefCountBaseNTS<ImageDesc, StatRender_Text_Mem>
{
    ...
    //Ptr<DisplayObjectBase>      pSpriteShape;
    Ptr<Image>              pImage;
...
class TextFormat : public RefCountBase<TextFormat, StatRender_Text_Mem>
{
    // Ranges:
    // fontSize         [0..2500] pts
...
public:
    ...
    TextFormat(const TextFormat& srcfmt, MemoryHeap* pheap = NULL)
        : FontList((pheap) ? pheap:srcfmt.GetHeap(), srcfmt.FontList),
...
class ParagraphFormat : public RefCountBaseNTS<ParagraphFormat, StatRender_Text_Mem>
{
public:
    friend class HashFunctor;
    struct HashFunctor
...
protected:
    ...
    UInt16 PresentMask;
public:
    ParagraphFormat():pTabStops(NULL), BlockIndent(0), Indent(0), Leading(0), 
        LeftMargin(0), RightMargin(0), PresentMask(0) 
    {}

    ParagraphFormat(const ParagraphFormat& src) : pTabStops(NULL), 

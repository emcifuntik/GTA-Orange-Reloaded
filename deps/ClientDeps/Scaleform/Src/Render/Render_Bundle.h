...
Notes       :   
History     :   May 5, 2010 - Refactored to be independent from render
                tree nodes. Changed naming from batch to bundle.
...
struct SortKeyInterface
{
    ...
    // and 
    virtual bool UpdateBundleEntry(SortKeyData, BundleEntry*, TreeCacheRoot*, Renderer2DImpl*) { return false; }
...
public:
    ...
    // Key-type sepecific constructors (External)
    SortKey(PrimitiveFill* fill);
    SortKey(MeshProvider* fill);    
    SortKey(SortKeyMaskType maskType);
    SortKey(SortKeyType blendKeyType, BlendMode mode);
    SortKey(SortKeyTextType t);
    ...
    bool UpdateBundleEntry(BundleEntry* e, TreeCacheRoot* tr, Renderer2DImpl* r)
    { return pImpl->UpdateBundleEntry(Data, e, tr, r); }
...
public:
    ...
    BundleEntry(TreeCacheNode* node, const SortKey& key)
    ...
    bool UpdateBundleEntry(TreeCacheRoot* tr, Renderer2DImpl* r);

...
MemoryBufferImage::MemoryBufferImage(ImageFormat format, const ImageSize& size, unsigned use,
 ...
 : Render::MemoryBufferImage(format,size, use, textureArena, sync,
...
MemoryBufferImage::MemoryBufferImage(ExtraData* exd, ImageFormat format, const ImageSize& size, unsigned use,
 ...
 : Render::MemoryBufferImage(format,size, use, textureArena, sync, data, len),
                                     pExtraData(exd)
{ 
    if (pExtraData) 
    {
        ...
        if (pExtraData)
            return SF_HEAP_NEW(args.GetHeap())
                MemoryBufferImage(pExtraData, GetFormat(), GetSize(),
                                  args.Use, args.TextureArena, args.GetUpdateSync(),
...
bool WrapperImageSource::ReadHeader()
{
    ...
    MemoryBufferImage* img = static_cast<MemoryBufferImage*>(pDelegate.GetPtr());
    pOriginalInput = FileReader::Instance.CreateSwfJpeg2HeaderOnly(img->GetData(), img->GetDataSize());
...
Render::MemoryBufferImage* FileReader::CreateMemoryBufferImage
    (File* file, const ImageCreateArgs& args, UInt64 length)
{
    ImageSize sz; 
    return SF_HEAP_NEW(args.GetHeap()) MemoryBufferImage(Image_None, sz,
        args.Use, args.TextureArena, args.GetUpdateSync(),
        file, file->LTell(), (UPInt)length);
}

Render::ImageSource* FileReader::CreateWrapperImageSource(Render::MemoryBufferImage* memImage) const
{

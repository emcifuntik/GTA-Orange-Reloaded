                                            ...
                                            HAL* hal, MeshCache* cache)
{
    ...
    if (pSourceVFormat)
    {
        pHal->MapVertexFormat(
            ...
            &pSingleVFormat, &pBatchVFormat, &pInstancedVFormat);
        ...
        SF_ASSERT(pSingleVFormat == pInstancedVFormat || pInstancedVFormat == NULL);
    }
    else
    {
...
void PrimitivePrepareBuffer::batchConvertStep()
{
    ...
    unsigned                  batchVertexSize = pBatchVFormat->Size;
    ...
    for (meshIndex = 0; meshIndex < originalConvertMeshCount; meshIndex++)
    {
        ...
        {
            tailRepeatCount++;
            if (instancingSupported && (tailRepeatCount >= params.MaxBatchInstances))
            {
                meshIndex++;
                break;
            }
        ...
        if ((pmesh->LargeMesh) || (tailMeshType == PrimitiveBatch::DP_Failed))
        {
    ...
    {
        // Create a new chunk with this data.
        if ((tailMeshType != PrimitiveBatch::DP_Failed) && (tailRepeatCount > 1))

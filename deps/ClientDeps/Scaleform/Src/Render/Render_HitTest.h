...
//  ----------------------------------------------------------
//  bool HitTestShape(const ShapeDataInterface& shape, 
        ...
        if ((styles[0] == 0) != (styles[1] == 0))
        {
            ...
            while((edgeType = shape.ReadEdge(&pos, coord)) != Edge_EndPath)
            {
                ...
                if(edgeType == Edge_QuadTo)
                {
                    ...
                    if (Math2D::CheckCurveIntersection(x1, y1, x2, y2, x3, y3, x, y))
                    {
                        styleCount ^= 1;
                    }
                    lastX = coord[2];
    ...
    for (i = 0; i < gen->mStrokeSorter.GetPathCount(); ++i)
    {
        ...
        for (j = 0; j < n; )
        {
            ...
            if (v1.segType == Math2D::Seg_QuadTo)
            {
                ...
                Math2D::TessellateQuadCurve(&gen->mPath, tol, v1.x, v1.y, v2.x, v2.y);
            }
        }
                         ...
                         StrokeGenerator* gen, const ToleranceParams& tol)
{
    ...
    for (i = 0; i < gen->mStrokeSorter.GetPathCount(); ++i)
    {
        ...
        for (j = 0; j < n; )
        {
            ...
            {
                const StrokeSorter::VertexType& v2 = gen->mStrokeSorter.GetVertex(i, j++);
                Math2D::TessellateQuadCurve(&gen->mStroker, tol, v1.x, v1.y, v2.x, v2.y);
            }
        }
        gen->mStroker.FinalizePath(&gen->mPath);
...
#else
    return HitTestRoundJoinStroke(shape, pos, strokeStyleIdx, trans, 1.0f, x, y, gen, tol);
#endif
}

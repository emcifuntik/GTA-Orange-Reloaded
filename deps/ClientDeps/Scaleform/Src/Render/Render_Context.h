...
protected:
    ...
    inline EntryPage*   getEntryPage() const;
    ...
    static void*    badRenderData() { return (void*)0xbad; }
...
public:
    ...
    inline const EntryData* GetReadOnlyData() const;
    inline const EntryData* GetDisplayData() const;
    ...
    inline void*        GetRenderData() const
    { SF_ASSERT(pRenderer != badRenderData()); return pRenderer; }
    inline void         SetRenderData(void* p)
    { SF_ASSERT(p != badRenderData()); pRenderer = p; }
    inline void*        GetRenderDataClean() const
...
public:
    ...
    virtual void    EntryChanges(ChangeBuffer&) { }
...
{               
    ...
    inline void     AddEntriesToList(List<Entry> *plist);
    inline void     RemoveEntriesFromList(List<Entry> *plist);
};
...
struct SnapshotPage : public ListNode<SnapshotPage>
{
    ...
    void            MarkAsDestroyed(unsigned index)     { *((UPInt*)&pData[index]) |= 1; }
    bool            IsDestroyed(unsigned index) const   { return *((UPInt*)&pData[index]) & 1; }
...
private:
    ...
    Lock*    getLock()  { return &pCaptureLock->LockObject; }
    ...
    void        destroyNativeNodes(List<Entry>& destroyList);
...
const EntryData* Entry::GetReadOnlyData() const
{    
    return EntryRef(this).GetData();
}
const EntryData* Entry::GetDisplayData() const
{    
...
#define SF_RENDER_CONTEXT_ENTRY_INLINES(class) \
    inline NodeData*       GetWritableData(unsigned cb) { return (class*)Entry::getWritableData(cb); }     \
    inline const NodeData* GetReadOnlyData() const  { return (const class*)Entry::GetReadOnlyData(); } \
    inline const NodeData* GetDisplayData() const   { return (const class*)Entry::GetDisplayData(); }

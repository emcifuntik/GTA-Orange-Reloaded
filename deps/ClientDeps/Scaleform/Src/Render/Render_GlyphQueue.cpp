...
GlyphNode* GlyphQueue::evictOldSlot(unsigned w, unsigned h, unsigned pass)
{
    ...
    while (!SlotQueue.IsNull(slot))
    {
        if (slot->PinCount == 0)
        {
            ...
            GlyphSlot* to = slot;
            GlyphBand* band = slot->pBand;
            while (!band->Slots.IsNull(to) && to->PinCount == 0)
            {
...
void GlyphQueue::VisitGlyphs(GlyphQueueVisitor* visitor) const
{
    ...
    while(!SlotQueue.IsNull(slot))
    {
        ...
        visitor->Visit(0, uvRect, slot->PinCount ? GlyphQueueVisitor::Rect_SlotPinned : GlyphQueueVisitor::Rect_Slot);
        slot = slot->pNext;
    }

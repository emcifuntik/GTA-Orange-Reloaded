...
{
    SF_COMPILER_ASSERT(sizeof(this) <= Pool_DataUnitSize);
        ...
        if (!matrices[i].IsNull())
        {
            if (matrices[i].Has3D())
            {
                ...
                if (matrices[i].HasCxform())
                {
                    ...
                    SF_ASSERT(cx.M[1][0] == values[i] + 2);
                    SF_ASSERT(cx.M[2][0] == values[i] + 2);
                    SF_ASSERT(cx.M[3][0] == values[i] + 2);
                    SF_ASSERT(cx.M[0][1] == values[i] + 3);
                    SF_ASSERT(cx.M[3][1] == values[i] + 3);
                    SF_UNUSED(cx);
                }
                SF_UNUSED(m);
            }
        ...
        case Mode_SetCxform:
            for(i = 0; i < k; i++)
            {
             //   SF_ASSERT((iteration != 252) || (i != 23));
                ...
                {
                    Cxform cx;
                    cx.M[0][0] = cx.M[1][0] = cx.M[2][0] = cx.M[3][0] = (float)values[j] + 2;
                    cx.M[0][1] = cx.M[1][1] = cx.M[2][1] = cx.M[3][1] = (float)values[j] + 3;

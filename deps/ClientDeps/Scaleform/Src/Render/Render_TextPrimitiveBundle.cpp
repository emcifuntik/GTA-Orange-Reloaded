...
{    
    ...
    {
        if (Layers[i]->RemoveEntry(e) && (Layers[i]->GetMeshCount() == 0))
        {
            Layers.RemoveAt(i);
            i--;
        }
...
public: 
    ...
    {
        SF_AMP_SCOPE_RENDER_TIMER("SKI_TextPrimitive::DrawBundleEntry");
...
SortKey::SortKey(SortKeyTextType)
{

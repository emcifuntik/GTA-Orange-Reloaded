...
void SF_STDCALL RescaleImageData(ImageData& dest, ImageData& src, ImageRescaleType resizeType)
{
    ...
    for (unsigned iplane = 0; iplane < planeCount; iplane++)
    {
        ...
        ResizeImageBilinear(dplane.pData, dplane.Width, dplane.Height, dplane.Pitch,
                            splane.pData, splane.Width, splane.Height, splane.Pitch,
                            resizeType);
    }
}
                                  ...
                                  Palette* pcolorMap, void* scanlineArg)
{
    ...
    UPInt   bytesPerPixel = ImageData::GetFormatBytesPerPixel(format, formatPlaneIndex);
    ...
    for (UPInt j = 0; j < scanlineCount; j++, psource += splane.Pitch, pdest += dplane.Pitch)
        copyScanline(pdest, psource, bytesPerPixel * splane.Width, pcolorMap, scanlineArg);
}
...
static ScanlineConvert ImageScanlineConvertTable[] = 
{
    ...
    { Image_P8,       Image_A8,       Image_CopyScanline_P_RGB},

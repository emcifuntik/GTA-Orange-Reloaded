...
bool GradientData::SetRecordCount(UInt16 count)
{
    ...
    GradientRecord* newRecs = (GradientRecord*)SF_HEAP_AUTO_ALLOC(this, sizeof(GradientRecord)*count);
    if (newRecs == 0)
        return 0;
...
//-----------------------------------------------------------------------
UPInt GradientData::GetHashValue() const
{
    unsigned    i;
    UPInt   hashCode = Type;
...
//-----------------------------------------------------------------------
GradientImage::GradientImage(PrimitiveFillManager* mng, GradientData* data, unsigned texArena) :
    pManager(mng),
    pData(data),
    Size(1, 1),
    TextureArena(texArena)
{
    if (data)
    {
...
bool GradientImage::Decode(ImageData* pdest, CopyScanlineFunc copyScanline, void* arg) const
{
    ...
    //--------------------
    GradientRamp ramp(pData->GetRecords(), pData->GetRecordCount(), 
                      pData->IsLinearRGB() ? 2.17f : 1.0f);
    ...
    unsigned i, j, size;
    if (pData->GetGradientType() == GradientLinear)
    {
    ...
    else
    {
        ...
        if (pData->GetGradientType() == GradientFocalPoint)
        {
            gr.Init(radius, pData->GetFocalRatio() * radius, 0);
        }

        dst += dplane.Pitch;
        for (j = 1; j < size; j++)
        {
            if (pData->GetGradientType() == GradientRadial)
            {
                for (i = 1; i < size; i++)
                {                       
                    y = j - center + 0.5f;
            ...
            {
                SF_ASSERT(pData->GetGradientType() == GradientFocalPoint);
                for (i = 1; i < size; i++)
                {
                    y = j - center + 0.5f;
                    x = i - center + 0.5f;

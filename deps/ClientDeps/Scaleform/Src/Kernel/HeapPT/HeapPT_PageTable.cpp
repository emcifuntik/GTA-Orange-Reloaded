...
bool PageTable::RemapRange(void* mem, UPInt newSize, UPInt oldSize)
{
    ...
    DebugNode* node = GetDebugNode(UPInt(mem));
#endif
...
void PageTable::SetDebugNode(UPInt address, DebugNode* node)
{
    ...
    UPInt i5  = address >> Heap_Root_PageShift;
    HeapHeaderRoot*  rootHeader = RootTable + i5;
...
DebugNode* PageTable::GetDebugNode(UPInt address) const
{
    ...
    SF_HEAP_ASSERT(rootHeader->HasTable(address));
...
DebugNode* PageTable::FindDebugNode(UPInt address) const
{
    ...
    const HeapHeader1* table1 = rootHeader->GetTable(i4)->GetTable(i3)->GetTable(i2)->GetTable(i1);

    if (table1->pDebugNode)
        return table1->pDebugNode;

    const HeapSegment* thisSeg = table1->pSegment;

    // Second case: debug node is placed in the same Lv2 table.
    const HeapHeader2* table2 = rootHeader->GetTable(i4)->GetTable(i3)->GetTable(i2);

    for(UPInt k1 = i1; k1 >= 0; --k1)
    {
        const HeapHeader1* table1 = table2->GetTable(k1);
    ...
    {
        const HeapHeader5* table5 = RootTable + k5;

        for (UPInt k4 = 0; k4 <= i4; ++k4)
        {
            const HeapHeader4* table4 = table5->GetTable(k4);

            for (UPInt k3 = 0; k3 <= i3; ++k3)
            {
                const HeapHeader3* table3 = table4->GetTable(k3);

                for (UPInt k2 = 0; k2 <= i2; ++k2)
                {
                   const HeapHeader2* table2 = table3->GetTable(k2);

                   for (UPInt k1 = 0; k1 < Heap_Lv1_TableSize; ++k1)
                   {     
                       const HeapHeader1* table1 = table2->GetTable(k1);

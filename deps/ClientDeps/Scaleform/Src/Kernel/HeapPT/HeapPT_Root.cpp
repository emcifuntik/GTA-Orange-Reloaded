...
//------------------------------------------------------------------------
void SysAllocGranulator::VisitSegments(SegVisitor* visitor, 
                                       unsigned catSeg, 
                                       unsigned catUnused) const
{
...
void SysAllocWrapper::VisitSegments(Heap::SegVisitor* visitor, 
                                    unsigned catSeg, unsigned catUnused) const
{
...
int HeapRoot::DestroyArena(UPInt arena)
{
    ...
    {
        SF_DEBUG_MESSAGE1(!empty, "MemoryHeap Arena %d is not empty!", arena);
...
//------------------------------------------------------------------------
#ifdef SF_MEMORY_TRACE_ALL
void HeapRoot::OnAlloc(const MemoryHeapPT* heap, UPInt size, UPInt align, unsigned sid, const void* ptr)
{
    LockSafe::Locker lock(&RootLock);
    pTracer->OnAlloc(heap, size, align, sid, ptr);
}

void HeapRoot::OnRealloc(const MemoryHeapPT* heap, const void* oldPtr, UPInt newSize, const void* newPtr)
{
    LockSafe::Locker lock(&RootLock);
    pTracer->OnRealloc(heap, oldPtr, newSize, newPtr);
}

void HeapRoot::OnFree(const MemoryHeapPT* heap, const void* ptr)
{
    LockSafe::Locker lock(&RootLock);

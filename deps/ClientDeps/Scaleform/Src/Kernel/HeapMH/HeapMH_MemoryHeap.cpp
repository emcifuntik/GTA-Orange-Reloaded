...
//------------------------------------------------------------------------
void MemoryHeap::ReleaseRootHeapMH() // static
{
    ...
    Memory::pGlobalHeap->dumpMemoryLeaks();
...
//------------------------------------------------------------------------
void MemoryHeapMH::freeLocked(void* ptr)
{
...
//------------------------------------------------------------------------
inline void* MemoryHeapMH::allocMem(UPInt size, const AllocInfo* info)
{
...
//------------------------------------------------------------------------
inline void* MemoryHeapMH::allocMem(UPInt size, UPInt align, const AllocInfo* info)
{
    HeapMH::PageInfoMH pageInfo;
    void* ptr = pEngine->Alloc(size, align, &pageInfo);

#ifdef SF_MEMORY_TRACE_ALL
    Heap::GlobalRootMH->OnAlloc(this, size, align, 0, ptr);
#endif
...
inline void* MemoryHeapMH::allocMem(HeapMH::PageInfoMH* parentInfo, const void *thisPtr, 
                                    UPInt size, const AllocInfo* info)
{
    HeapMH::PageInfoMH pageInfo;
    void* ptr = pEngine->Alloc(size, &pageInfo);
...
//------------------------------------------------------------------------
void  MemoryHeapMH::freeMem(HeapMH::PageMH* page, void* ptr)
{
#ifdef SF_MEMORY_ENABLE_DEBUG_INFO
    HeapMH::PageInfoMH pageInfo;
...
//------------------------------------------------------------------------
void  MemoryHeapMH::freeMem(HeapMH::NodeMH* node, void* ptr)
{
#ifdef SF_MEMORY_ENABLE_DEBUG_INFO
    HeapMH::PageInfoMH pageInfo;
...
//------------------------------------------------------------------------
void* MemoryHeapMH::reallocMem(HeapMH::PageMH* page, void* oldPtr, UPInt newSize)
{
#ifdef SF_MEMORY_ENABLE_DEBUG_INFO
...
//------------------------------------------------------------------------
void* MemoryHeapMH::reallocMem(HeapMH::NodeMH* node, void* oldPtr, UPInt newSize)
{
#ifdef SF_MEMORY_ENABLE_DEBUG_INFO
    HeapMH::PageInfoMH oldPageInfo;
...
void MemoryHeapMH::GetRootStats(RootStats* stats)
{
    ...
    stats->SysMemFootprint      = GetTotalFootprint()
        + stats->UserDebugFootprint; 
    stats->SysMemUsedSpace      = GetTotalUsedSpace()
        + stats->UserDebugFootprint; 
...
void SysAlloc::shutdownHeapEngine()
{
    MemoryHeap::ReleaseRootHeapMH();

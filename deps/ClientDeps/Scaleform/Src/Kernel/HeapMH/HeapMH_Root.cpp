...
//------------------------------------------------------------------------
#ifdef SF_MEMORY_TRACE_ALL
void RootMH::OnAlloc(const MemoryHeapMH* heap, UPInt size, UPInt align, unsigned sid, const void* ptr)
{
    Lock::Locker lock(&RootLock);
    pTracer->OnAlloc(heap, size, align, sid, ptr);
}

void HeapRoot::OnRealloc(const MemoryHeapMH* heap, const void* oldPtr, UPInt newSize, const void* newPtr)
{
    Lock::Locker lock(&RootLock);
    pTracer->OnRealloc(heap, oldPtr, newSize, newPtr);
}

void HeapRoot::OnFree(const MemoryHeapMH* heap, const void* ptr)
{
    Lock::Locker lock(&RootLock);

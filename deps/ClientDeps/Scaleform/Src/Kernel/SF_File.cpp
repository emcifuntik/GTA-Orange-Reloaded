...
BufferedFile::BufferedFile() : DelegatedFile(0)
{
    ...
    pBuffer     = (UByte*)SF_MEMALIGN(FILEBUFFER_SIZE, 32, Stat_Default_Mem);
    BufferMode  = NoBuffer;
    FilePos     = 0;
    Pos         = 0;
    DataSize    = 0;
...
int     BufferedFile::Tell()
{
    ...
    if (pos!=-1)
    {
        SF_ASSERT(BufferMode != ReadBuffer);
...
int     BufferedFile::Read(UByte *pdestBuffer, int numBytes)
{
    if ( (BufferMode==ReadBuffer) || SetBufferMode(ReadBuffer))
    {
        ...
        while (numBytes > 0)
        {
            ...
            if (((int)DataSize-(int)Pos)>=numBytes)
            {
...
{    
    if (BufferMode == ReadBuffer)
    {
        if (origin == Seek_Cur)
        {
            ...
            origin = Seek_Set;
            SF_ASSERT(((FilePos - DataSize + Pos) + (UInt64)offset) < ~(UInt64)0);
        ...
        else if (origin == Seek_Set)
        {
            ...
            {
                SF_ASSERT((FilePos-DataSize) < ~(UInt64)0);
    ...
    {
        //SF_ASSERT((FilePos - DataSize) >= (FilePos - DataSize + Pos + offset));
        Pos += offset;
        SF_ASSERT(int (Pos) >= 0);
        return int (FilePos - DataSize + Pos);
    }
...
bool    BufferedFile::ChangeSize(int newSize)
{
    FlushBuffer();

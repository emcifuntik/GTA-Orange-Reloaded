...
#define SF_MEMORY_REDEFINE_NEW_IMPL(class_name, check_delete, StatType)                                  \
    ...
    void*   operator new(UPInt sz, MemoryHeap* heap)                                                     \
    { void *p = SF_HEAP_ALLOC(heap, sz, StatType); return p; }                                           \
    void*   operator new(UPInt sz, MemoryHeap* heap, int blocktype)                                      \
    { SF_UNUSED(blocktype); void *p = SF_HEAP_ALLOC(heap, sz, blocktype); return p; }                    \
    void*   operator new(UPInt sz, MemAddressPtr adr)                                                    \
    { void *p = Memory::AllocAutoHeap(adr, sz, AllocInfo(StatType,__FILE__,__LINE__)); return p; }       \
    void*   operator new(UPInt sz, const char* pfilename, int line)                                      \
    { void* p = Memory::Alloc(sz, AllocInfo(StatType, pfilename, line)); return p; }                     \
    void*   operator new(UPInt sz, MemoryHeap* heap, const char* pfilename, int line)                    \
    { void* p = Memory::AllocInHeap(heap, sz, AllocInfo(StatType, pfilename, line)); return p; }         \
    void*   operator new(UPInt sz, MemAddressPtr adr, const char* pfilename, int line)                   \
    { void* p = Memory::AllocAutoHeap(adr, sz, AllocInfo(StatType, pfilename, line)); return p; }        \
    void*   operator new(UPInt sz, int blocktype, const char* pfilename, int line)                       \
    { void* p = Memory::Alloc(sz, AllocInfo(blocktype, pfilename, line)); return p; }                    \
    void*   operator new(UPInt sz, MemoryHeap* heap, int blocktype, const char* pfilename, int line)     \
    { void* p = Memory::AllocInHeap(heap, sz, AllocInfo(blocktype, pfilename, line)); return p; }        \
    void*   operator new(UPInt sz, MemAddressPtr adr, int blocktype, const char* pfilename, int line)    \
    { void* p = Memory::AllocAutoHeap(adr, sz, AllocInfo(blocktype, pfilename, line)); return p; }       \
    ...
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemoryHeap*)                           \
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemoryHeap*, int)                      \
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemoryHeap*, const char*, int)         \
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemoryHeap*, int,const char*,int)      \
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemAddressPtr)                         \
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemAddressPtr, int)                    \
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemAddressPtr, const char*, int)       \
    { check_delete(class_name, p); SF_FREE(p); }                            \
    void    operator delete(void *p, MemAddressPtr,int,const char*,int)     \
    { check_delete(class_name, p); SF_FREE(p); }

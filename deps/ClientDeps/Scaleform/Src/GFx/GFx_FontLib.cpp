...
{    
    ...
    else if (pweakLib)
    {
        ...
        Loader loader(pstates->GetFileOpener(), pstates->GetZlibSupport());
        loader.SetResourceLib(pweakLib->GetResourceLib());

        Ptr<MovieDefBindStates> bs = *new MovieDefBindStates(pstates);
        Ptr<LoadStates> pls = *new LoadStates(loader.GetLoaderImpl(), pstates, bs);    

        // Get movieDef. Most of the time this will just look up MovieDefImpl
        // in the library without performing the actual binding.
        pfontDefImpl = 
            *loader.GetLoaderImpl()->CreateMovie_LoadState(pls, pdataDef, Loader::LoadWaitCompletion);
    }
    if (!pfontDefImpl)
        return 0;

...
void    MovieDataDef::LoadTaskData::InitImageFileMovieDef(
    unsigned fileLength, ImageResource *pimageResource, ImageCreator* imgCreator, bool bilinear)
{
    SF_ASSERT(LoadState == LS_Uninitialized);
    ...
    Ptr<ImageShapeCharacterDef> shapeDef = *SF_HEAP_NEW(pHeap) ImageShapeCharacterDef(pimageResource, imgCreator);
...
{
    SF_UNUSED(plog);
    SF_ASSERT(LoadingFrame >= 0 && LoadingFrame < Header.FrameCount);
...
void    MovieDataDef::LoadTaskData::SetLoadingPlaylistFrame(const Frame& frame)
{
...
void    MovieDataDef::LoadTaskData::SetLoadingInitActionFrame(const Frame& frame)
{
    Lock::Locker lock(&PlaylistLock);
...
ResourceKey  MovieDataDef::CreateMovieFileKey(const char* pfilename,
                                                    SInt64 modifyTime,
                          ...
                          iimport++, pimportData = pimportData->pNext)
        {
                    ...
                    (sourceURL.GetSize() - fontlibKeySize), fontlibKey) == 0))
            {
                ...
                if (pdefImpl)
                {
                    ...
                    Ptr<FontLib> pfontLib = pdefImpl->GetFontLib();
                    if (pfontLib.GetPtr() 
...
//                     bd.pResource->GetResourceUse() == Resource::Use_Bitmap)
//                 {
//                     ImageResource* pimageRes = (ImageResource*) (Resource*) bd.pResource;
//                     pImagePacker->Add(presData, pimageRes->GetImageInfo());
//                 }
...
Movie* MovieDefImpl::CreateInstance(const MemoryParams& memParams, bool initFirstFrame, ActionControl* actionControl)    
{
    ...
    Movie* movieView = CreateInstance(memContext, initFirstFrame, actionControl);
    memContext->Release();  // the MovieView owns the context now
    return movieView;
}


Movie* MovieDefImpl::CreateInstance(MemoryContext* memContext, bool initFirstFrame, ActionControl* actionControl)
{
    ASSupport* pASSupport = GetASSupport();
    if (pASSupport == NULL)
    {

...
{
    SF_ASSERT(LoadingFrame >= 0 && LoadingFrame < FrameCount);
...
void TimelineSnapshot::SourceTags::Unpack(GFxPlaceObjectBase::UnpackedData& data) const
{
    ...
    if (pTextFilterTag != pMainTag)
    {
        if (pDepthTag == pTextFilterTag) dataIdx = 0;
        else if (pCharIdTag == pTextFilterTag) dataIdx = 1;
        else if (pMatrixTag == pTextFilterTag) dataIdx = 2;
        else if (pClassNameTag == pTextFilterTag) dataIdx = 3;
        else if (pCxFormTag == pTextFilterTag) dataIdx = 4;
        else if (pBlendModeTag == pTextFilterTag) dataIdx = 5;
        else if (pClipDepthTag == pTextFilterTag) dataIdx = 6;
        else if (pRatioTag == pTextFilterTag) dataIdx = 7;
        else { pTextFilterTag->Unpack(unpackedData[8]); dataIdx = 8; }
        data.Pos.TextFilter = unpackedData[dataIdx].Pos.TextFilter;
        data.Pos.SetTextFilterFlag();
    }
}
...
{   
    ...
    if (pin->IsVerboseParse())
    {
        ...
        if (po3Flags & PlaceObject3Tag::PO3_HasClassName)
        {
            ...
            pos.ClassName = pin->ReadString(pin->GetHeap());
        }
        ...
        {
            pos.SetTextFilterFlag();
            FilterDesc filters[FilterDesc::MaxFilters];
            LoadFilters(pin, filters, FilterDesc::MaxFilters);
        }
...
void PlaceObject3Tag::Unpack(PlaceObjectTag::UnpackedData& data)
{
    ...
    if (po3Flags & PO3_HasFilters)
    {
        ...
        FilterDesc filters[FilterDesc::MaxFilters];
        unsigned numFilters = LoadFilters(&sc, filters, FilterDesc::MaxFilters);
        if (numFilters)
        {
            if (!data.Pos.TextFilter)
                data.Pos.TextFilter = *SF_HEAP_NEW_ID(Memory::GetHeapByAddress(this), StatMD_Tags_Mem) TextFilter;

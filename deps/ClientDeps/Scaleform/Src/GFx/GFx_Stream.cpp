...
void    Stream::ReadCxformRgb(Render::Cxform *pcxform)
{
    ...
    if (hasMult)
    {
        ...
        pcxform->M[1][0] = ReadSInt(nbits) / 256.0f;
        pcxform->M[2][0] = ReadSInt(nbits) / 256.0f;
        pcxform->M[3][0] = 1.0f;
    }
    else
    {
        for (unsigned i = 0; i < 4; i++) { pcxform->M[i][0] = 1.0f; }
    }
    if (hasAdd)
    {
        pcxform->M[0][1] = (float) ReadSInt(nbits);
        pcxform->M[1][1] = (float) ReadSInt(nbits);
        pcxform->M[2][1] = (float) ReadSInt(nbits);
        pcxform->M[3][1] = 1.0f;
    }
    else
    {
        for (unsigned i = 0; i < 4; i++) { pcxform->M[i][1] = 0.0f; }
    }
    pcxform->Normalize();
}
...
void    Stream::ReadCxformRgba(Render::Cxform *pcxform)
{
    ...
    if (hasMult)
    {
        ...
        pcxform->M[1][0] = ReadSInt(nbits) / 256.0f;
        pcxform->M[2][0] = ReadSInt(nbits) / 256.0f;
        pcxform->M[3][0] = ReadSInt(nbits) / 256.0f;
    }
    else
    {
        for (int i = 0; i < 4; i++) { pcxform->M[i][0] = 1.0f; }
    }
    if (hasAdd)
    {
        pcxform->M[0][1] = (float) ReadSInt(nbits);
        pcxform->M[1][1] = (float) ReadSInt(nbits);
        pcxform->M[2][1] = (float) ReadSInt(nbits);
        pcxform->M[3][1] = (float) ReadSInt(nbits);
    }
    else
    {
        for (int i = 0; i < 4; i++) { pcxform->M[i][1] = 0.0f; }
    }       
    pcxform->Normalize();
}

/**********************************************************************
Filename    :   GFx/AS2/AS2Support.cpp
                                               ...
                                               bool debugHeap)
{
    ...
    desc.Limit  = AS2::MemoryContextImpl::HeapLimit::INITIAL_DYNAMIC_LIMIT; // 128K is the initial dynamic limit
    ...
    CharacterDef::CharacterDefType type)
{
    ...
    case CharacterDef::Shape:
        return SF_HEAP_AUTO_NEW(pparent) AS2::GenericDisplayObj(pcharDef, proot->pASMovieRoot, pparent, rid);
    ...
    (UnpackedData& data, StreamContext& sc, const UByte* prawdata, UInt8 version)
{
    ...
    if (!peh)
    {
        ...
        if (u32Flags)
            ...
            allFlags = sc.ReadU16();
        SF_UNUSED(allFlags);
        peh = SF_HEAP_NEW_ID(Memory::GetHeapByAddress(this), StatMD_Tags_Mem) PlaceObjectTag::EventArrayType;
        // Read SwfEvents.
        for (;;)
        {
            // Read event.
            ...
            SwfEvent*    ev = SF_HEAP_NEW_ID(Memory::GetHeapByAddress(this), StatMD_Tags_Mem) SwfEvent;
...
{
    pbuttonDef->ButtonActions.Resize(pbuttonDef->ButtonActions.GetSize() + 1);
    Ptr<AS2::ButtonAction> pact = *SF_HEAP_NEW_ID(p->GetLoadHeap(), StatMD_CharDefs_Mem) AS2::ButtonAction;
    pbuttonDef->ButtonActions.Back() = pact;

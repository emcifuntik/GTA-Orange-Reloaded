...
void    DisplayList::SwapEntriesAtIndexes(DisplayObjectBase* owner, UPInt origIndex1, UPInt origIndex2)
{
    ...
    SwapRenderTreeNodes(owner, origIndex1, origIndex2);
}
...
InteractiveObject* DisplayList::GetDisplayObjectByName(const ASString& name, bool caseSensitive) const
{
    if (name.IsEmpty())
        return NULL;
    ...
    if (caseSensitive)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            SF_ASSERT(p);
            if (p && p->IsInteractiveObject() && p->CharToInteractiveObject_Unsafe()->GetName() == name)
            {
                pch = p;
                break;
            }
    ...
    {   // Case-insensitive version.
        ...
        for (i = 0; i < n; i++)
        {
            DisplayObjectBase* p = GetDisplayObject(i);
            SF_ASSERT(p);
            if (p && p->IsInteractiveObject() && name.Compare_CaseInsensitive_Resolved(p->CharToInteractiveObject_Unsafe()->GetName()))
            {
                pch = p;
                break;
            }
    ...
    // Only InteractiveObject's have names.
    SF_ASSERT((pch == 0) || pch->IsInteractiveObject());
    if (pch)
        pCachedChar = pch->CharToInteractiveObject_Unsafe();
    else
        pCachedChar = NULL;
    return pCachedChar;
}
...
void    DisplayList::AddDisplayObject(DisplayObjectBase* owner, const CharPosInfo &pos, DisplayObjectBase* ch, UInt32 addFlags)    
{
    ...
    else
    {
        ...
        SF_ASSERT(!(size > 0 && (index >= 0 && index < size) && 
...
{   
    ...
    ch->SetBlendMode((pos.HasBlendMode()) ? (DisplayObjectBase::BlendType)pos.BlendMode : poldCh->GetBlendMode());
    ch->SetRatio(pos.Ratio);
    ch->SetClipDepth(pos.ClipDepth);    
    if (pos.TextFilter)
        ch->SetFilters(*pos.TextFilter);
...
void    DisplayList::RemoveFromRenderTree(DisplayObjectBase* owner, UPInt index)
{
    ...
    else
    {
        ...
        // need to update tree indices for all entries after the one being inserted...
        for (UPInt i = index + 1; i < DisplayObjectArray.GetSize() && DisplayObjectArray[i].TreeIndex == dobj.TreeIndex; ++i)
        {
        ...
        else
        {
            ...
            if (prevDE.GetClipDepth() != 0 || prevDE.MaskTreeIndex != ~0u)
            {
                ...
                if (ch->GetDepth() <= maskEntry.GetClipDepth() && ch->GetDepth() > maskEntry.GetDepth())
                {
                    ...
                    {
                        // a wierd special case when mask is inside the mask is possible when swapDepths is used.
...
void DisplayList::SwapRenderTreeNodes(DisplayObjectBase* owner, UPInt index1, UPInt index2)
{
...
//         if (de1.GetClipDepth() != 0 && de2.GetClipDepth() != 0)
//         {
//             // both are masks: just swap mask nodes
//             SF_ASSERT(par1->HasMask() && par2->HasMask());
//             par1->SetMaskNode(NULL);
//             par2->SetMaskNode(n1);
//             par1->SetMaskNode(n2);
//         }
    }
        ...
        for (unsigned i = 0, trIdx = 0, maskIdx = ~0u; i < DisplayObjectArray.GetSize(); ++i)
        {
            ...
            if ((de.GetDepth() >= 0 || de.GetDepth() == INVALID_DEPTH) && de.TreeIndex != ~0u)
            {
                ...
                else
                {
                    ...
                    if (pcontainerNode->GetAt(de.TreeIndex)->HasMask())
                    {

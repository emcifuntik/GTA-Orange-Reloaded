...
void Object::ForEachChild_GC() const
{
    ...
    ResolveHandler.template ForEachChild_GC<Functor>();
    if (pWatchpoints)
    {
...
void Object::ExecuteForEachChild_GC(OperationGC operation) const
{
    ASRefCountBaseType::CallForEachChild<Object>(operation);
}
...
static const NameFunction GAS_ObjectFunctionTable[] = 
{
    ...
    { "watch",                  &ObjectProto::Watch },
    { "unwatch",                &ObjectProto::Unwatch },
...
void ObjectProto::Watch(const FnCall& fn)
{
    SF_ASSERT(fn.ThisPtr);
...
void ObjectProto::Unwatch(const FnCall& fn)
{
    SF_ASSERT(fn.ThisPtr);

...
void MouseCtorFunction::ForEachChild_GC() const
{
    SetCursorTypeFunc.template ForEachChild_GC<Functor>();
    if (pListenersArray)
        Functor::Call(pListenersArray);
    CFunctionObject::template ForEachChild_GC<Functor>();
}
SF_GC_PREGEN_FOR_EACH_CHILD(MouseCtorFunction)

void MouseCtorFunction::ExecuteForEachChild_GC(OperationGC operation) const
{
    ASRefCountBaseType::CallForEachChild<MouseCtorFunction>(operation);
}
...
void    MouseCtorFunction::GetTopMostEntity(const FnCall& fn)
{
    ...
    else 
    {
        ...
        if (proot->GetMainMovie())
        {
            // x and y vars are in _root coords. Need to transform them
            // into world coords
            Matrix2F m = proot->GetMainMovie()->GetWorldMatrix();
            PointF a(x, y);

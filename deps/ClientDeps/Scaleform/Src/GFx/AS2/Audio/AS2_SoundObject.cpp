...
void SoundProto::Attach(const FnCall& fn)
{
    ...
    if (fn.NArgs < 1)
    {
        fn.Env->LogScriptError("Sound.AttachSound needs one Argument (the file name)");
        return;
    }
    ASString id(fn.Arg(0).ToString(fn.Env));
    SoundObject* pobj = static_cast<SoundObject*>(fn.ThisPtr);
...
{ 
    if (fn.NArgs < 1)
    {
        fn.Env->LogScriptError("Sound.SetPan needs one Argument");
        return;
    }
...
{ 
    if (fn.NArgs < 1)
    {
        fn.Env->LogScriptError("Sound.SetVolume needs at least one argument");
        return;
    }
...
static const NameFunction GAS_SoundFunctionTable[] = 
{

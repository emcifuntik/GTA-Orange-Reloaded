...
void KeyCtorFunction::Update(UInt32 code, UByte ascii, UInt32 wcharCode, UInt8 keyboardIndex)
{
    SF_ASSERT(keyboardIndex < GFX_MAX_CONTROLLERS_SUPPORTED);
...
void KeyCtorFunction::NotifyListeners(InteractiveObject *, UInt32 code, UByte ascii, 
                                      UInt32 wcharCode, int eventId, UInt8 keyboardIndex)
{
    States[keyboardIndex].LastKeyCode         = code;
    States[keyboardIndex].LastAsciiCode       = ascii;
    States[keyboardIndex].LastWcharCode       = wcharCode;
...
#else // SF_NO_GC
    // CC version
    ASString handlerName = EventId_GetFunctionName(ToAS2Root(pMovieRoot)->GetStringManager(), EventId(UByte(eventId)));

...
protected:
    ...
    {
        BaseClass::template ForEachChild_GC<Functor>();
        Constructor.template ForEachChild_GC<Functor>();
        __Constructor__.template ForEachChild_GC<Functor>();
        if (pInterfaces)
        {
    ...
    virtual void ExecuteForEachChild_GC(RefCountBaseGC<StatMV_ActionScript_Mem>::OperationGC operation) const
    {
        ASRefCountBaseType::template CallForEachChild<Prototype<BaseClass, Environment> >(operation);
    }
    void Finalize_GC()
    {
        if (pInterfaces)
...
public:
    ...
    static void Watch(const FnCall& fn);
    static void Unwatch(const FnCall& fn);
...
protected:
    ...
    void ForEachChild_GC() const
    {
        ...
        if (SuperProto)
            Functor::Call(SuperProto);
        if (SavedProto)
            Functor::Call(SavedProto);
        Constructor.template ForEachChild_GC<Functor>();
    }
    virtual void ExecuteForEachChild_GC(OperationGC operation) const
    {
        ASRefCountBaseType::CallForEachChild<SuperObject>(operation);
    }

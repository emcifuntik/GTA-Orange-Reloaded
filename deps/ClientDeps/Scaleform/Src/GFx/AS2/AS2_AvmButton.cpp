        ...
        // and only for onRoll/Drag/Over/Out events
    {
        ...
        if (pparent)
        {
            ...
            if (pparentSprite)
            {
                ...
                for (unsigned i = 0; i < pdef->ButtonActions.GetSize(); i++)
                {
                    const ButtonAction* action = static_cast<const ButtonAction*>(pdef->ButtonActions[i].GetPtr());
...
void AvmButton::RecreateCharacters(ButtonRecord::MouseState mouseState)
{
    ...
    for (unsigned i = 0; i < Button::StatesCount; ++i)
    {
        ...
        ArrayLH<Ptr<DisplayObjectBase> >& chars = 
        ...
        if (chars.GetSize() == 0)
        {
            ...
            for (UPInt j = 0; j < pdef->ButtonRecords.GetSize(); ++j)
            {
                ButtonRecord* rec = &pdef->ButtonRecords[j];
                    ...
                    (i == Button::Hit && rec->IsHitTest()))
                {
                    ...
                    if (ccinfo.pCharDef)
                    {
                        ...
                        Ptr<DisplayObjectBase>  ch = *GetAS2Root()->GetASSupport()->CreateCharacterInstance
                            (GetMovieImpl(), ccinfo.pCharDef, GetButton(), rec->CharacterId, ccinfo.pBindDefImpl);
                        chars.PushBack(ch);
                        ...
                        if (rec->mTextFilter)
                            ch->SetFilters(*rec->mTextFilter.GetPtr());

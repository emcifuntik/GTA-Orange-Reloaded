...
void ArrayObject::ExecuteForEachChild_GC(OperationGC operation) const
{
    ASRefCountBaseType::CallForEachChild<ArrayObject>(operation);
}

template <class Functor>
void ArrayObject::ForEachChild_GC() const
{
    ...
    for(i = 0, n = Elements.GetSize(); i < n; i++)
    {
        ...
        if (pval)
            pval->template ForEachChild_GC<Functor>();
    }
}

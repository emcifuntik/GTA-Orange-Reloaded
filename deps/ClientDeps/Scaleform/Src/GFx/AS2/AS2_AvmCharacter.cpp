    ...
    case M_xmouse:  
        {
            ...
            MovieImpl& mi = *GetMovieImpl();
            Render::PointF a = mi.GetMouseState(0)->GetLastPosition();

            float w = mi.VisibleFrameRect.Width();
            float h = mi.VisibleFrameRect.Height();
            float nsx = 2.f * ((a.x - PixelsToTwips(mi.ViewOffsetX)) / w) - 1.0f;
            float nsy = 2.f * ((a.y - PixelsToTwips(mi.ViewOffsetY)) / h) - 1.0f;
            GetMovieImpl()->ScreenToWorld.SetNormalizedScreenCoords(nsx, nsy);
            Render::PointF b;
            pDispObj->TransformPointToLocal(b, a, false);
            ...
            if (!method.IsNull())
            {
                ...
                if (env->CheckExtensions())
                {
                    ...
                    if (handlerFound)
                    {
                        ...
                        if (id.Id & EventId::Event_AuxEventMask || EventId::Event_DragOver || 

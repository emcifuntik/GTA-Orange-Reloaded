...
struct ValueProperty : public ASRefCountBase<ValueProperty>
{
    ...
    {
        GetterMethod.template ForEachChild_GC<Functor>();
        SetterMethod.template ForEachChild_GC<Functor>();
    }
    virtual void ExecuteForEachChild_GC(OperationGC operation) const
    {
        ASRefCountBaseType::CallForEachChild<ValueProperty>(operation);
    }
    void Finalize_GC() 
    {
...
//        UInt32          Dummy;
    ...
    void    Sub (Environment* penv, int v2)   { SetNumber(ToNumber(penv) - (Number)v2); }

    void    Add (Environment* penv, const Value& v);
    void    Sub (Environment* penv, const Value& v)   { SetNumber(ToNumber(penv) - v.ToNumber(penv)); }
    void    Mul (Environment* penv, const Value& v)   { SetNumber(ToNumber(penv) * v.ToNumber(penv)); }

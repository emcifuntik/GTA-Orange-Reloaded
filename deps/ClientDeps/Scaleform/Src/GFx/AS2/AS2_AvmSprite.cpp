...
Filename    :   GFx/AS2/AvmSprite.cpp
    ...
    InteractiveObject* origChar)
{
    ...
    Ptr<DisplayObjectBase> ch = *GetAS2Root()->GetASSupport()->CreateCharacterInstance
        (GetMovieImpl(), ccinfo.pCharDef, GetSprite(), pos.CharacterId, ccinfo.pBindDefImpl);
    SF_ASSERT(ch);
...
void    AvmSprite::AdvanceFrame(bool nextFrame, float framePos)
{
    ...
    if (GetMovieImpl()->IsMouseSupportEnabled())
        spr->DisplayObjContainer::DoMouseDrag();
...
bool    AvmSprite::GetMember(Environment* penv, ASStringContext *psc, const ASString& name, Value* pval)
{
    ...
    InteractiveObject* pch = GetSprite()->GetDisplayObjectByName(name, ASEnvironment.IsCaseSensitive());
    if (pch)
    {
        // Found object.
...
InteractiveObject* AvmSprite::GetRelativeTarget(const ASString& name, bool first_call)
{
    ...
    return GetSprite()->GetDisplayObjectByName(name, e.IsCaseSensitive());
}

void    AvmSprite::SetLevel(int level)
{
    GetMovieImpl()->GetRenderRoot()->Add(GetSprite()->GetRenderNode());
    Level = level;
...
bool    AvmSprite::ExecuteEvent(const EventId& id)
{
    ...
    if (id == EventId::Event_Unload)
    {
        ...
        SF_ASSERT(!GetSprite()->IsOptAdvancedListFlagSet());
...
bool     AvmSprite::IsFocusEnabled() const
{
...
void AvmSprite::SpriteAttachAudio(const FnCall& fn)
{
    ...
    if (obj)
    {
        if (obj->GetObjectType() == Object::Object_NetStream)
        {
            ...
            if (pvideoState)
                pvideoState->AttachAudio(obj, psprite);
        }
        /*
        else if (obj->GetObjectType() == Object::Object_Microphone)
        {
        }
        */
    }
...
void AvmSprite::SpriteHitTest(const FnCall& fn)
{
    ...
    if (fn.NArgs >= 2)
    {
        ...
        bool has3D = false;
        for (unsigned movieIndex = 0; movieIndex < proot->MovieLevels.GetSize(); movieIndex++)
        {
            if (proot->MovieLevels[movieIndex].pSprite->Is3D())
            {
                has3D = true;
                break;
            }
        ...
        if ( proot && has3D )
        {
            // map stage coords to window coords
            float wx = (float)fn.Arg(0).ToNumber(fn.Env);
            float wy = (float)fn.Arg(1).ToNumber(fn.Env);

            wx = (wx - proot->ViewOffsetX) / proot->ViewScaleX;
            wy = (wy - proot->ViewOffsetY) / proot->ViewScaleY;

            fn.Result->SetBool(proot->HitTest(wx, wy, 
                (hitTestMask & DisplayObject::HitTest_IgnoreInvisible) ? Movie::HitTest_ShapesNoInvisible : Movie::HitTest_Shapes)); 
            return;
        }
    ...
    else if (fn.NArgs == 1)
    {
        ...
        if (arg.IsCharacter())
        {
            InteractiveObject* pchar   = arg.ToCharacter(fn.Env);
            ptarget = pchar ? pchar->CharToSprite() : 0;
        }
        else
        {
            ...
            {
                InteractiveObject* pchar = result.ToCharacter(fn.Env);
                ptarget = pchar ? pchar->CharToSprite() : 0;
            }
        }
        if (!ptarget)
                                            ...
                                            const Value& val)
{
    ...
    if (spr && name.IsBuiltin())
    {
        ...
        {
            spr->SetRendererString(val.ToString(avmSpr->GetASEnvironment()));
        }

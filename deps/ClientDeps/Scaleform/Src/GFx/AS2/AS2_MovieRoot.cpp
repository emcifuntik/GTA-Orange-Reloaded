...
Filename    :   GFx/AS2/MovieRoot.cpp
...
void MovieRoot::GenerateMouseEvents(unsigned mouseIndex)
{
        ...
        if ( ((changeMask >> buttonIdx) & 0x1) != 0 )
        {
            ...
            if ((ms.GetPrevButtonsState() & buttonMask) == 0)
            {
                ...
                if ((ms.GetButtonsState() & buttonMask))
                {
                    ...
                    // onPress
                    if (ActiveEntity)
                        ActiveEntity->OnMouseEvent(ButtonEventId(genAuxEvent ? EventId::Event_PressAux : EventId::Event_Press, mouseIndex, 0, buttonIdx));
        ...
        else if (ms.GetButtonsState() & buttonMask)
        {
                ...
                || ActiveEntity->GetTrackAsMenu())
            {
                    ...
                    && TopmostEntity->GetTrackAsMenu())
                {
                    ...
                    ActiveEntity = TopmostEntity;
                    ActiveEntity->OnMouseEvent(ButtonEventId(genAuxEvent ? EventId::Event_DragOverAux : EventId::Event_DragOver, 
                        mouseIndex, 
    ...
    if (ms.GetButtonsState() == 0 && TopmostEntity != ActiveEntity)
    {
        ...
        if (ActiveEntity)
            ActiveEntity->OnMouseEvent(ButtonEventId
            (EventId::Event_RollOver, mouseIndex, ActiveEntity->IncrementRollOverCnt()));
...
*/
    ...
    Ptr<AudioBase> audiostate = pMovieImpl->pStateBag->GetAudio();
    if (audiostate)
        audiostate->RegisterASClasses(*pGlobalContext, sc);
#endif
#ifdef GFX_ENABLE_VIDEO
    Ptr<Video::VideoBase> videostate = pMovieImpl->pStateBag->GetVideo();
    if (videostate)
...
/*
    ...
    for (; i< pMovieImpl->MovieLevels.GetSize(); i++)
    {
        if (pMovieImpl->MovieLevels[i].Level >= level)
        {           
            if (pMovieImpl->MovieLevels[i].Level == level)
            {
                SF_DEBUG_WARNING1(1, "MovieRoot::SetLevelMovie fails, level %d already occupied", level);
                return 0;
            }           
            // Found insert spot.
            break;              
    ...
    G_SetFlag<MovieImpl::Flag_LevelClipsChanged>(pMovieImpl->Flags, true);

    // Insert the item.
    MovieImpl::LevelInfo li;
    li.Level  = level;
    li.pSprite= psprite;
    pMovieImpl->MovieLevels.InsertAt(i, li);

    psprite->OnInsertionAsLevel(level);

    if (level == 0)
    {
        pMovieImpl->SetMainMovie(psprite);
        SetMovieDefImpl(psprite->GetResourceMovieDef());
        pMovieImpl->pStateBag->SetDelegate(GetMovieDef()->pStateBag);
        // Frame timing
...
//             }
    ...
    if (pmovieDef)
    {
        ...
        pnewChar = *static_cast<Sprite*>(GetASSupport()->CreateCharacterInstance(
                        ...
                        pparent, newCharId, pmovieDef, CharacterDef::Sprite));
        pnewChar->SetLoadedSeparately();
            
//            *SF_HEAP_NEW(GetMovieHeap()) Sprite(pmovieDef->GetDataDef(), pmovieDef, this,
...
//             pnewChar = *(Sprite*)ccinfo.pCharDef->CreateCharacterInstance(pparent, newCharId,
//                 ccinfo.pBindDefImpl);
            pnewChar = *(Sprite*)pASSupport->CreateCharacterInstance
                (GetMovieImpl(), ccinfo.pCharDef, pparent, newCharId, ccinfo.pBindDefImpl, CharacterDef::Sprite);
        }
    ...
    Environment* penv = GetAvmLevelMovie(0)->GetASEnvironment();
    SF_ASSERT(penv);
    //SF_ASSERT(pentry->MovieClipLoaderHolder.ToObject()->GetObjectType() == ObjectInterface::Object_MovieClipLoader);
    MovieClipLoader* pmovieClipLoader = 
...
#else
    SF_UNUSED5(p_entry, pls, data, fileLen, succeeded);
#endif
}
...
void    MovieRoot::ProcessLoadVars(LoadQueueEntry *p_entry, LoadStates* pls)
{
    ...
    if (pentry->URL.GetLength())
    {
        ...
        else
        {
            ...
            if (pfile)
            {
                ...
                {
                    if (!MovieImpl::ReadLoadVariables(pfile, &data, &fileLen))
                        pentry->LoadVarsHolder.DropRefs();
                }
                else 
                {
    ...
    ProcessLoadVarsMT(pentry, pls, data, fileLen, true);
}
...
Sprite* MovieRoot::CreateEmptySprite(LoadStates* pls, int level)
{
    ...
    ResourceKey createKey = MovieDataDef::CreateMovieFileKey("", 0, 0, 0, 0);
...
bool MovieRoot::AttachDisplayCallback(const char* pathToObject, void (SF_CDECL *callback)(void*), void* userPtr)
{   
    if (!GetLevelMovie(0))
        return 0;
    ...
    {
        InteractiveObject*     pchar   = obj.ToCharacter(penv);
                                    ...
                                    unsigned, FocusMovedType fmt, ProcessFocusKeyInfo*)
{
    ...
    {
        if (!newFocus || !newFocus->DoesAcceptMouseFocus())
            return false;
    }
    return true;
}
...
BytesLoaded(0), FirstFrameLoaded(false)
{
    ...
    {
        MovieRoot* as2mr = static_cast<MovieRoot*>(pMovieImpl->pASMovieRoot);
        SF_ASSERT(qe->Type & GFxAS2LoadQueueEntry::LTF_LevelFlag);
...
bool GFxAS2LoadQueueEntryMT_LoadMovie::LoadFinished()
{
    ...
    GFxAS2LoadQueueEntry* qe = static_cast<GFxAS2LoadQueueEntry*>(pQueueEntry);
    MovieRoot*   as2mr = ToAS2Root(pMovieImpl);
    if (btaskDone)
    {
        ...
        Sprite* pmovie0 = as2mr->GetLevelMovie(0);
        ...
        if (!pdefImpl)
        {
            ...
            if (qe->pCharacter)
            {
                ...
                CharacterCreateInfo ccinfo =
                    pparent->GetResourceMovieDef()->GetCharacterCreateInfo
                    (ResourceId(CharacterDef::CharId_EmptyMovieClip));
//                 Ptr<Sprite> emptyChar = *(Sprite*)ccinfo.pCharDef->CreateCharacterInstance(pparent, NewCharId,
//                     ccinfo.pBindDefImpl);
                Ptr<Sprite> emptyChar = *(Sprite*)as2mr->pASSupport->CreateCharacterInstance
                    (pMovieImpl, ccinfo.pCharDef, pparent, NewCharId, ccinfo.pBindDefImpl, CharacterDef::Sprite);
                //AB: we need to replicate the same CreateFrame, Depth and the Name.
                // Along with replicated Id this is important to maintain the loaded
                // movie on the stage. Otherwise, once the parent clip reaches the end
                // of its timeline the original movieclip will be recreated. Flash
                ...
                pOldChar = as2mr->GetLevelMovie(qe->Level);
        ...
        else 
        {
            ...
            if (!CharSwitched)
            {
                ...
                Ptr<Sprite> pnewChar = *static_cast<Sprite*>(as2mr->GetASSupport()->CreateCharacterInstance(
                    as2mr->GetMovieImpl(), pdefImpl->GetDataDef(), 
...
//                     as2mr, pparent, NewCharId, true);
            ...
            if (extensions && !FirstFrameLoaded && ((pdefImpl->pBindData->GetBindState() & MovieDefImpl::BSF_Frame1Loaded) != 0))
            {
                ...
                if (qe->Level == -1)
                    ...
                    psprite = as2mr->GetLevelMovie(qe->Level);
            ...
            if (pdefImpl->pBindData->GetBindState() & MovieDefImpl::BS_Finished)
            {
                ...
                if (qe->Level == -1)
                {
                    ...
                    psprite = pOldChar->CharToSprite();
                    as2mr->ResolveStickyVariables(pOldChar);
                }
                else
                    psprite = as2mr->GetLevelMovie(qe->Level);
                SF_ASSERT(psprite.GetPtr());
...
ASBuiltinType   EventId_GetFunctionNameBuiltinType(const EventId& evt) 
{
    ...
    unsigned idx;
    if (evt.Id >= EventId::Event_Invalid && evt.Id <= EventId::Event_LastCombined)

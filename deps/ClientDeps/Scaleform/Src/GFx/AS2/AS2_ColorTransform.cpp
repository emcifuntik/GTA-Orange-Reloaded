...
#include "AS2/AS2_NumberObject.h"
...
{
    mColorTransform = ct;
    mColorTransform.M[0][1] *= 255;
    mColorTransform.M[1][1] *= 255;
    mColorTransform.M[2][1] *= 255;
    mColorTransform.M[3][1] *= 255;
}
...
bool ColorTransformObject::GetMember(Environment *penv, const ASString &name, Value* val)
{
    ...
    {
        *val = mColorTransform.M[2][0];
        return true;
    }
    else if (name == "alphaMultiplier")
    {
        *val = mColorTransform.M[3][0];
        return true;
    }
    ...
    {
        *val = mColorTransform.M[2][1];
        return true;
    }
    else if (name == "alphaOffset")
    {
        *val = mColorTransform.M[3][1];
        return true;
    }
    else if (name == "rgb")
    {
        Value ro, go, bo;
        UInt8 rc = (UInt8) mColorTransform.M[0][1];
        UInt8 gc = (UInt8) mColorTransform.M[1][1];
        UInt8 bc = (UInt8) mColorTransform.M[2][1];
...
bool ColorTransformObject::SetMember(Environment *penv, const ASString &name, const Value &val, const PropFlags& flags)
{
    ...
    {
        mColorTransform.M[2][0] = static_cast<float>(val.ToNumber(penv));
        return true;
    }
    else if (name == "alphaMultiplier")
    {
        mColorTransform.M[3][0] = static_cast<float>(val.ToNumber(penv));
        return true;
    }
    else if (name == "redOffset")
    {
    ...
    {
        mColorTransform.M[2][1] = static_cast<float>(val.ToNumber(penv));
        return true;
    }
    else if (name == "alphaOffset")
    {
        mColorTransform.M[3][1] = static_cast<float>(val.ToNumber(penv));
        return true;
    }
    else if (name == "rgb")
    {
        ...
        mColorTransform.M[1][0] = 0;
        mColorTransform.M[2][0] = 0;
        ...
        mColorTransform.M[0][1] = static_cast<float>(rc);
        mColorTransform.M[1][1] = static_cast<float>(gc);
        mColorTransform.M[2][1] = static_cast<float>(bc);
        return true;
    }
    ...
    if (fn.NArgs > 0)
    {
        ...
        if (NULL != p)
        {
            ...
            else
            {
                ...
                ct.M[1][0] = static_cast<float>(c[GFxColorTransform_GreenMultiplier].ToNumber(fn.Env));
                ct.M[2][0] = static_cast<float>(c[GFxColorTransform_BlueMultiplier].ToNumber(fn.Env));
                ct.M[3][0] = static_cast<float>(c[GFxColorTransform_AlphaMultiplier].ToNumber(fn.Env));
                ct.M[0][1] = static_cast<float>(c[GFxColorTransform_RedOffset].ToNumber(fn.Env));
                ct.M[1][1] = static_cast<float>(c[GFxColorTransform_GreenOffset].ToNumber(fn.Env));
                ct.M[2][1] = static_cast<float>(c[GFxColorTransform_BlueOffset].ToNumber(fn.Env));
                ct.M[3][1] = static_cast<float>(c[GFxColorTransform_AlphaOffset].ToNumber(fn.Env));
...
void ColorTransformProto::ToString(const FnCall& fn)
{
    ...
    ASString ps[8] = 
    {
        ...
        Value(ct.M[2][0]).ToString(fn.Env, 6),
        Value(ct.M[3][0]).ToString(fn.Env, 6),
        Value(ct.M[0][1]).ToString(fn.Env, 6),
        Value(ct.M[1][1]).ToString(fn.Env, 6),
        Value(ct.M[2][1]).ToString(fn.Env, 6),
        Value(ct.M[3][1]).ToString(fn.Env, 6)
    };
...
void ColorTransformCtorFunction::GlobalCtor(const FnCall& fn)
{
    if (fn.ThisPtr && fn.ThisPtr->GetObjectType() == Object::Object_ColorTransform)
    {
        ...
        if (fn.NArgs > 7)
        {
            ...
            ct.M[1][0] = static_cast<float>(fn.Arg(1).ToNumber(fn.Env));
            ct.M[2][0] = static_cast<float>(fn.Arg(2).ToNumber(fn.Env));
            ct.M[3][0] = static_cast<float>(fn.Arg(3).ToNumber(fn.Env));
            ct.M[0][1] = static_cast<float>(fn.Arg(4).ToNumber(fn.Env));
            ct.M[1][1] = static_cast<float>(fn.Arg(5).ToNumber(fn.Env));
            ct.M[2][1] = static_cast<float>(fn.Arg(6).ToNumber(fn.Env));
            ct.M[3][1] = static_cast<float>(fn.Arg(7).ToNumber(fn.Env));
        }
    }
}

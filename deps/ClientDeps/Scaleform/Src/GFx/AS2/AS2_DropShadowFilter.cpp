...
: BitmapFilterObject(penv, FilterDesc::Filter_DropShadow)
{
}
...
bool DropShadowFilterObject::SetMember(Environment* penv, const ASString& name, const Value& val, const PropFlags& flags /* = PropFlags */)
{
    ...
    else if (name == "inner")
    {
        ...
        if (plog)
            plog->LogWarning("DropShadowFilter.inner property is not supported!");
        return true;
    }
    ...
    else if (name == "quality")
    {
        Log* plog = penv->GetLog();
        if (plog)
            plog->LogWarning("DropShadowFilter.quality property is not supported!");
        return true;
    }

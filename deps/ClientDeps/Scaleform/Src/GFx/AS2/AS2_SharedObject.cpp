...
Filename    :   AS2/GFxSharedObject.cpp
...
void SharedObject::Flush(Environment* penv, GFxSharedObjectVisitor* writer)
{
    if (!writer)
        return;
    ...
    struct DataWriter : public ObjectInterface::MemberVisitor
    {
        ...
        DataWriter(Environment *penv, GFxSharedObjectVisitor* pwriter)
        {
            pEnv = penv;
            pWriter = pwriter;
        }
            ...
            case Value::BOOLEAN:
                {
                    // writeout boolean                    
                    pWriter->AddProperty( propname, 
                        val.ToBool(pEnv) ? "true" : "false", Value::VT_Boolean);
                    break;
                }
            case Value::NULLTYPE:
                {
                    // writeout null
                    pWriter->AddProperty( propname, "", Value::VT_Null);
                    break;
                }
            case Value::OBJECT:
                {
...
void SharedObjectProto::Clear(const FnCall &fn)
{
    ...
    if (psoMgr)
    {
        Ptr<GFxSharedObjectVisitor> pwriter = *psoMgr->CreateWriter(pthis->GetName(), 
...
void SharedObjectCtorFunction::ForEachChild_GC() const
{
    ...
    {   
        ...
        pobj.template ForEachChild_GC<Functor>();
        ++it;
    }
}
SF_GC_PREGEN_FOR_EACH_CHILD(SharedObjectCtorFunction)

void SharedObjectCtorFunction::ExecuteForEachChild_GC(OperationGC operation) const
{
    ASRefCountBaseType::CallForEachChild<SharedObjectCtorFunction>(operation);
}

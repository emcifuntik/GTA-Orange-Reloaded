...
#include "AS2/AS2_StringObject.h"

#include "AS2/AS2_ArrayObject.h"
#include "AS2/AS2_NumberObject.h"
#include "AS2/AS2_BooleanObject.h"
#include "AS2/AS2_Math.h"
#include "AS2/AS2_Timers.h"
#include "AS2/AS2_AsBroadcaster.h"
#include "AS2/AS2_Date.h"
#include "AS2/AS2_KeyObject.h"
#include "AS2/AS2_Mouse.h"
#include "AS2/AS2_ExternalInterface.h"
#include "AS2/AS2_Selection.h"
#include "AS2/AS2_TextFormat.h"
#include "AS2/AS2_SharedObject.h"
#include "AS2/AS2_AmpMarker.h"

#include "AS2/AS2_TextSnapshot.h"
...
#include "AS2/AS2_Capabilities.h"
#include "AS2/AS2_LoadVars.h"
...
#include "AS2/AS2_DropShadowFilter.h"
#include "AS2/AS2_GlowFilter.h"
...
#include "AS2/AS2_FunctionRefImpl.h"
...
static void GAS_GetIMECandidateListStyle(const FnCall& fn)
{
    ...
    InteractiveObject* const poriginalTarget   = fn.Env->GetTarget();   
    MovieImpl* proot                     = poriginalTarget->GetMovieImpl();
    Ptr<IMEManager> pimeMgr             = proot->GetIMEManager();
    if (pimeMgr)
    {
        IMECandidateListStyle st; 
...
//#if !defined(SF_NO_FXPLAYER_AS_IME) && defined(GFX_ENABLE_IME) 
...
ActionBufferData* ActionBufferData::CreateNew()
{
    ...
    return SF_HEAP_NEW_ID(Memory::GetGlobalHeap(), StatMD_ActionOps_Mem) ActionBufferData();
}
...
void    ActionBufferData::Read(Stream* in, unsigned actionLength)
{
    ...
    pBuffer = (UByte*)SF_HEAP_AUTO_ALLOC_ID(this, BufferLen, StatMD_ActionOps_Mem);
...
{
    psc->Align();
    pBuffer = (UByte*)SF_HEAP_AUTO_ALLOC_ID(this, eventLength, StatMD_ActionOps_Mem);
...
void ExecutionContext::Function1OpCode(ActionBuffer* pActions)
{
    ...
    if (AmpServer::GetInstance().IsEnabled() && !name.IsEmpty())
    {
        ...
        pEnv->GetMovieImpl()->AdvanceStats->RegisterScriptFunction(pActions->GetBufferData()->GetSwdHandle(), 
            ...
            name.ToCStr(), funcObj->GetLength(), 2);
    }
#endif  // SF_AMP_SERVER
}
                        ...
                        ExecuteType execType)
{
    ...
    AMP::ScopeFunctionTimer funcTimer(shouldProfile ? viewStats : NULL, pBufferData->GetSwdHandle(), 
        startPc + pBufferData->GetSWFFileOffset());
...
{   
    return static_cast<MovieRoot*>(Target->GetMovieImpl()->pASMovieRoot);
}
...
void LocalFrame::ForEachChild_GC() const
{
    ASStringHash_GC<Value>::ConstIterator it = Variables.Begin();
    while(it != Variables.End())
    {   
        const Value& value = it->Second;
        value.template ForEachChild_GC<Functor>();
        ++it;
    }
    if (PrevFrame)
        Functor::Call(PrevFrame);
    Callee.template ForEachChild_GC<Functor>();
    Caller.template ForEachChild_GC<Functor>();
}
SF_GC_PREGEN_FOR_EACH_CHILD(LocalFrame)

void LocalFrame::ExecuteForEachChild_GC(OperationGC operation) const
{
    ASRefCountBaseType::CallForEachChild<LocalFrame>(operation);
}

...
void Value::ForEachChild_GC() const
{
    if (IsFunctionObject())
        V.FunctionValue.template ForEachChild_GC<Functor>();
    else if (IsObject() && V.pObjectValue)
        Functor::Call(V.pObjectValue);
    else if (IsProperty())
        Functor::Call(V.pProperty);
}
...
SInt32  Value::ToInt32(Environment* penv) const
{
    ...
    else
    {
        ...
        if (v == 0 || NumberUtil::IsNaNOrInfinity(v))
            return 0;

...
: BitmapFilterObject(penv, FilterDesc::Filter_Glow)
{
}
...
bool GlowFilterObject::SetMember(Environment* penv, const ASString& name, const Value& val, const PropFlags& flags /* = PropFlags */)
{
    ...
    else if (name == "inner")
    {
        ...
        if (plog)
            plog->LogWarning("GlowFilter.inner property is not supported!");
        return true;
    }
    ...
    else if (name == "quality")
    {
        Log* plog = penv->GetLog();
        if (plog)
            plog->LogWarning("GlowFilter.quality property is not supported!");
        return true;
    }

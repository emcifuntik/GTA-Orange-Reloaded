...
void    StyleSheetProto::GetStyle(const FnCall& fn)
{
    ...
    if (stylename.GetSize() > 0 && stylename[0] == '.')
        pstyle = pthis->CSS.GetStyle(Text::StyleKey::CSS_Class, stylename.ToCStr()+1, stylename.GetSize()-1);
    else
        pstyle = pthis->CSS.GetStyle(Text::StyleKey::CSS_Tag, stylename.ToCStr(), stylename.GetSize());
    if (!pstyle)
    {
        fn.Result->SetNull();
        return;
...
void    StyleSheetProto::GetStyleNames(const FnCall& fn)
{
    ...
    {
        temp.Clear();
        if (iter->First.Type == Text::StyleKey::CSS_Class)
            temp.AppendChar('.');
...
void    StyleSheetProto::SetStyle(const FnCall& fn)
{
    ...
    if (fn.NArgs < 2 || fn.Arg(1).IsNull())
    {
        ...
        {
            if (stylename[0] == '.')
                pthis->CSS.ClearStyle(Text::StyleKey::CSS_Class, stylename.ToCStr());
            else
                pthis->CSS.ClearStyle(Text::StyleKey::CSS_Tag, stylename.ToCStr());
        }
        return;
    }
    else
...
struct CSSTextFormatLoader : public Object::MemberVisitor
{
    ...
    void Visit(const ASString& name, const Value& val, UByte flags)
    {
        ...
        {
            if ( !SFstrncmp("true", pstr, (sz < 4) ? sz : 4) ) 
                pTFO->mTextFormat.SetKerning(false);        
            else if ( !SFstrncmp("false", pstr, (sz < 5) ? sz : 5) )

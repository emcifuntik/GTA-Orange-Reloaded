...
{    
    ...
    // name
    else if (IsAlpha(*pstr) || *pstr == '_' || (*pstr == '.'))
    {
...
#ifdef SF_CSS_VERBOSE_TRACE
        tokenString.Clear(); tokenString.AppendString(token.pBase, token.Length);
        SF_DEBUG_MESSAGE3(1, "   - Encountered '%s' @ %d,%d", tokenString.ToCStr(), 
...
void TextStyleParserHandler<T>::OpenCSSSelectorBlock(const CSSToken<T>& name, void* pdata)
{
    ...
    SF_DEBUG_MESSAGE1(1, "[CSS] Opening selector block: '%s'", sname.ToCStr());
#endif

    String keystr;
    // figure out key type
    StyleKey::KeyType type = StyleKey::CSS_Tag;
    if (name.Length > 0 && name.pBase[0] == '.')
    {
        type = StyleKey::CSS_Class;
                                                        ...
                                                        void* pdata)
{
    ...
    if (value.GetSize() > 0)
        ...
        - value[0].pBase);
    SF_DEBUG_MESSAGE2(1, "[CSS] Found property: '%s' => '%s'", sname.ToCStr(), sval.ToCStr());
#endif
    ...
    if      (sname == "color")              HandleColor(*parr, value);
        ...
        SF_DEBUG_MESSAGE1(1, "[CSS] Unknown property name '%s'", sname.ToCStr());
#endif
}

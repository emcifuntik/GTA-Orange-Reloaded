...
unsigned GFxMovieRoot::HandleKoreanIME(const IMEEvent& imeEvent)
{
    ...
    if (winEvt.IMEEvtType != IMEEvent::IME_PreProcessKeyboard)
    {
        ...
        if ((un.langId & 0xFFFF) == MAKELANGID(LANG_KOREAN, SUBLANG_KOREAN)) //korean
        {
            ...
            case WM_IME_STARTCOMPOSITION:
                {
                    Ptr<InteractiveObject> pfocusedCh = GetFocusedCharacter();
                    if (pfocusedCh && pfocusedCh->GetObjectType() == ObjectInterface::Object_TextField)
                    {
                        Ptr<GASTextField> ptextFld = static_cast<GASTextField*>(pfocusedCh.GetPtr());

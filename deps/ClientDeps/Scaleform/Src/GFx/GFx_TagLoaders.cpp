...
void UndoAndFilterPremultiplied(Render::Image* pimage)
{
    if (pimage->Format != Render::Image::Image_ARGB_8888) return;
    unsigned x,y;
    unsigned pitch = pimage->Width * 4 + 8;
        ...
        else
        {
            ...
            else 
            {
                ...
                else
                {
                    ...
                    Ptr<Render::MemoryBufferImage> img = *SF_HEAP_NEW(args.GetHeap()) MemoryBufferJpegImageWithZlibAlphas
                        (pzlib, jpegSize, Image_R8G8B8A8, Render::ImageSize(), args.Use, args.TextureArena, args.GetUpdateSync(),
                         file, file->LTell(), (UPInt)sz);
...
void    SF_STDCALL GFx_DefineShapeLoader(LoadProcess* p, const TagInfo& tagInfo)
{
    ...
    const PreprocessParams* pp = p->GetBindStates()->pPreprocessParams;
    if (pp)
    {
        SF_ASSERT(0); //@REN
//         RenderConfig cfg(pp->GetTessellateEdgeAA() ? 
//                                     RenderConfig::RF_EdgeAA : 0, 
//                                ~0U, ~0U);
...
void    SF_STDCALL GFx_FontTextureInfoLoader(LoadProcess* p, const TagInfo& tagInfo)
{
    ...
    Array<Ptr<TextureGlyph> > texGlyphsInTexture;
    ...
    for (i = 0; i < numTexGlyphs; ++i)
    {
        ...
        // create TextureGlyph
        Ptr<TextureGlyph> ptexGlyph = *SF_HEAP_NEW(pheap) TextureGlyph();
        ptexGlyph->SetImageResource(rh);
        ptexGlyph->UvBounds = RectF(uvBoundsLeft, uvBoundsTop,
                                              uvBoundsRight, uvBoundsBottom);
        ptexGlyph->UvOrigin = PointF(uvOriginX, uvOriginY);
    ...
    for (i = 0; i < numFonts; ++i)
    {
        ...
        Ptr<TextureGlyphData> ptextureGlyphData = pfd->GetTextureGlyphData();
...
void SF_STDCALL GFx_Scale9GridLoader(LoadProcess* p, const TagInfo& tagInfo)
{
    ...
    if (r.x1 >= r.x2)
    {
        p->LogWarning("Scale9Grid has negative width %f", (r.x2 - r.x1) / 20);
        return;
    }

    if (r.y1 >= r.y2)
    {
        p->LogWarning("Scale9Grid has negative height %f", (r.y2 - r.y1) / 20);
        return;
    }

...
//    if (id.GetIdIndex() != 286)
 //       return;
    ...
    Render::PointF initialTL = r.TopLeft();
...
//@REN
/*
void    StaticTextCharacter::Display(DisplayContext &context)   
{
    StaticTextDef* pdef = GetStaticTextDef();
    // And displays records.
    Matrix2F m = *context.pParentMatrix;
    m.Prepend(GetMatrix());

    Cxform cx = *context.pParentCxform;
    cx.Concatenate(GetCxform());

    // Do viewport culling if bounds are available.
    if (!FindMovieImpl()->GetVisibleFrameRectInTwips().Intersects(m.EncloseTransform(pdef->TextRect)))
        if (!(context.GetRenderFlags() & RenderConfig::RF_NoViewCull))
            return;

    GFX_RTREELOG_STARTOBJECT("StaticText", GetBounds(Render::Matrix2F::Identity), GetMatrix());

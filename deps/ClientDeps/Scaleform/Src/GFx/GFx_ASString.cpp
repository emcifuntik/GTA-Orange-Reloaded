...
ASStringNode* ASConstString::AppendCharNode(UInt32 ch) const
{
    ...
    return GetManager()->CreateStringNode(pNode->pData, pNode->Size, buff, index);
}
...
void        ASString::operator += (const ASString& str)
{
    AssignNode(GetManager()->CreateStringNode(pNode->pData, pNode->Size,
...
ASConstString ASConstString::operator + (const char* pstr) const
{
...
ASConstString ASConstString::operator + (const ASConstString& str) const
{
    return ASConstString(GetManager()->CreateStringNode(pNode->pData, pNode->Size,
        str.pNode->pData, str.pNode->Size));
}
    ...
    : pHeap(pheap)
{
    ...
    EmptyStringNode.HashFlags   = ASString::HashFunction("", 0) |
                                  ASString::Flag_ConstData | ASString::Flag_Builtin;
...
void    ASStringManager::AllocateStringNodes()
{
    StringNodePage* ppage = (StringNodePage*)
        SF_HEAP_ALLOC(pHeap, sizeof(StringNodePage), StatMV_ActionScript_Mem);
...
void   ASStringManager::AllocateTextBuffers()
{
    ...
    void*     pp    = SF_HEAP_ALLOC(pHeap, sizeof(TextPage) + 8, StatMV_ActionScript_Mem);    
...
{   
    ...
    return (char*)SF_HEAP_ALLOC(pHeap, length + 1, StatMV_ActionScript_Mem);
}

...
public:
    ...
    void Read(LoadProcess* p, TagType tagType, unsigned lenInBytes, bool withStyle);
...
void    ShapeSwfReader::Read(LoadProcess* p, TagType tagType, unsigned lenInBytes, bool withStyle)
{
    ...
    if (withStyle)
    {
        ...
        RectF bounds;
        in->ReadRect(&bounds); // actually, ignore; use ShapeMeshProvider to get bounds
        ...
        //else
        //    Shape.SetRectBoundsLocal(Shape.Bound);
...
//     Shape.Flags &= (~ShapeDataBase::Flags_TexturedFill);
//     for (i = 0, n = FillStyles.GetSize(); i < n; i++)
//     {
//         if (!FillStyles[i].IsSolid())
//         {
//             Shape.Flags |= ShapeDataBase::Flags_TexturedFill;
//             break;
//         }
//     }
    ...
    if (Shape.Flags & ShapeDataBase::Flags_StylesSupport)
    {
//         float maxStrokeExtent = 1.0f;
//         for(i = 0, n = StrokeStyles.GetSize(); i < n; i++)
//             maxStrokeExtent = Alg::Max(maxStrokeExtent, StrokeStyles[i].GetStrokeExtent());
//         
...
void    ShapeDataBase::ComputeBound(RectF* r) const
{
    ...
    SwfPathData::PathsIterator it = SwfPathData::PathsIterator(Paths, (Flags & Flags_Sfactor20) ? 1.0f/20.f : 1.0f);
    while(!it.IsFinished())
    {
        if (it.IsNewShape())
        {
        ...
        SwfPathData::EdgesIterator edgesIt = it.GetEdgesIterator();
        float ax, ay;
        ...
        {
            SwfPathData::EdgesIterator::Edge edge;
            edgesIt.GetEdge(&edge);
            if (edge.Curve)
            {
                float t, x, y;
                t = Render::Math2D::QuadCurveExtremum(ax, edge.Cx, edge.Ax);
                if (t > 0 && t < 1)
                {
                    Render::Math2D::PointOnQuadCurve(
                        ax, ay, 
                        edge.Cx, edge.Cy, 
                        edge.Ax, edge.Ay,
                        t, &x, &y);
                ...
                t = Render::Math2D::QuadCurveExtremum(ay, edge.Cy, edge.Ay);
                if (t > 0 && t < 1)
                {
                    Render::Math2D::PointOnQuadCurve(
                        ax, ay, 
...
//printf("%d", ret);
            return Render::HitTestFill<Matrix2F>(*this, Matrix2F::Identity, pt.x, pt.y);
        }
        else
            return true;
    }
...
//////////////////////////////////////////////////////////////////////////
void    ConstShapeNoStyles::Read(LoadProcess* p, TagType tagType, unsigned lenInBytes, bool withStyle)
{
    ShapeSwfReader reader(*this, p->GetPathAllocator());
    reader.Read(p, tagType, lenInBytes, withStyle);
}
...
{
    FillStyleType*   fs = reinterpret_cast<FillStyleType*>(o.Styles);
    StrokeStyleType* ss = reinterpret_cast<StrokeStyleType*>(fs + o.FillStylesNum);
...
void    ConstShapeWithStyles::Read(LoadProcess* p, TagType tagType, unsigned lenInBytes, bool withStyle)
{
    ShapeSwfReader reader(*this, p->GetPathAllocator());
    reader.Read(p, tagType, lenInBytes, withStyle);
...
void    ConstShapeWithStyles::SetStyles
    (unsigned fillStyleCount, FillStyleType* fillStyles, unsigned strokeStyleCount, StrokeStyleType* strokeStyles)
{
    if (Styles)
        SF_FREE(Styles);
...
void    ConstShapeWithStyles::BindResourcesInStyles(const GFx::ResourceBinding& resourceBinding)
{
    ...
    for (unsigned i = 0; i < FillStylesNum; ++i)
    {
        if (fillStyles[i].pFill && fillStyles[i].pFill->BindIndex != ~0u)
        {
            ...
            SF_ASSERT(rdata.pResource.GetPtr() != 0);
            SF_ASSERT(rdata.pResource->GetResourceType() == Resource::RT_Image);
...
RectF SwfShapeCharacterDef::GetBoundsLocal() const
{
    SF_ASSERT(pShapeMeshProvider);
...
//////////////////////////////////////////////////////////////////////////
ImageShapeCharacterDef::ImageShapeCharacterDef(ImageResource* pimage, ImageCreator* imgCreator)
{

...
class ImageLoader : public State
{
public:
    ImageLoader() : State(State_ImageLoader) { }

    // Image loading virtual function. purl should be encoded as UTF-8 to support
    // international file names.
// @IMG    virtual Render::ImageInfoBase* LoadImage(const char* purl) = 0;
};
*/
...
//@IMG      virtual ImagePacker*  Begin(ResourceId* pIdGen, ImageCreator* pic, ImageCreateInfo* pici) const = 0;
...
{
public:
//@IMG    virtual ImagePacker*  Begin(ResourceId* pIdGen, ImageCreator* pic, ImageCreateInfo* pici) const;
};
...
// @IMG    Render::ImageInfoBase*      CreateImageInfo(const ImageCreateInfo& info);   
...
public:
    ...
    //   static void Init(MemoryHeap* pheap = GMEMORY_HEAP_DEFAULT::CreateRootHeap());
    //   static void Destroy();
};

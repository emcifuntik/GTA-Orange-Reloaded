...
namespace Registry {
	...
	int CreateRegKeyStructure(HKEY hKey, LPCTSTR sPath)
	{
		...
		for (iPos = 0; iPos < iNameSz; iPos++)
		{
			...
			{
				sDir[iPos] = '\0';
				if (Registry::CreateRegistryKey(hKey, sDir))
				{
					iCount++;
				}
			}
	...
	bool DeleteRegistryKey(HKEY hKeyRoot, LPCTSTR pszSubKey)
	{
		...
		{
			HINSTANCE hLibInst = LoadLibrary(L"shlwapi.dll");
	...
	bool Get_StringRegistryValue(HKEY hKeyRoot, const char* pszSubKey, const char* pszValue, const char* pszBuffer, DWORD& rdwSize)
	{
		...
		lRes = RegQueryValueExA(hKey, pszValue, NULL, &dwType, (unsigned char*)pszBuffer, &rdwSize);
		if (lRes != ERROR_SUCCESS)
		{
			RegCloseKey(hKey);
			SetLastError(lRes);
	...
	bool Set_BOOLRegistryValue(HKEY hKeyRoot, LPCTSTR pszSubKey, LPCTSTR pszValue, bool bVal)
	{
		...
		return Registry::Set_DWORDRegistryValue(hKeyRoot, pszSubKey, pszValue, dwVal);
	}
	...
	bool Get_BOOLRegistryValue(HKEY hKeyRoot, LPCTSTR pszSubKey, LPCTSTR pszValue)
	{
		...
		if (Registry::Get_DWORDRegistryValue(hKeyRoot, pszSubKey, pszValue, dwVal))
		{
			return (dwVal > 0);
		}
		return false;

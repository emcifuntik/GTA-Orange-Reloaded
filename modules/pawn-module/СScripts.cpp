...
int AMXAPI CScripts::aux_LoadProgram(AMX* amx, char* filename)
{
	FILE* fp;
...
void CScripts::AMXPrintError(CGameMode* pGameMode, AMX *amx, int error)
{
	if (error != AMX_ERR_NONE)
	{
		API::Get().Print("Script[%s]: Run time error");//, (long)amx->curline);
	}
}

char * AMXAPI CScripts::aux_StrError(int errnum)
{
...
CScripts *CScripts::Get()
{
	if (!singleInstance) {
		singleInstance = new CScripts();
	}
